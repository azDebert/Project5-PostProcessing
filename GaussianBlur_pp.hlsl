//--------------------------------------------------------------------------------------
// Gaussian Blur Horizontal Post-Processing Pixel Shader
//--------------------------------------------------------------------------------------
...
// Post-processing shader that applies a horizontal Gaussian blur to the scene 
float4 main(PostProcessingInput input) : SV_Target
{
    ...
    // Horizontal Blur, so we only need to offset the x coordinate of the texture which is the xStep
    float xStep = 1.0f;
    float yStep = 0.0f;

    for (int i = 1; i < 5; i++)
    {
        float2 normalizedOffset = float2(xStep * offset[i] * gBlurAmount, yStep * offset[i]) / gViewportWidth;
