...
ID3D11PixelShader*  gGaussianBlurVerticalPostProcess = nullptr;
...
bool LoadShaders()
{
	...
	gGaussianBlurVerticalPostProcess = LoadPixelShader ("GaussianBlurVertical_pp");
	...
	if (gBasicTransformVertexShader == nullptr || gPixelLightingVertexShader == nullptr ||
		...
		g2DPolygonVertexShader      == nullptr || gGradientPostProcess       == nullptr ||
        gBlurPostProcess            == nullptr || gUnderwaterPostProcess     == nullptr ||
		gRetroPostProcess           == nullptr || gGaussianBlurPostProcess   == nullptr ||
		gGaussianBlurVerticalPostProcess == nullptr || gBloomPostProcess           == nullptr  )
	{
...
void ReleaseShaders()
{
	...
	if (gGradientPostProcess)             gGradientPostProcess            ->Release();
    ...
    if (gGaussianBlurVerticalPostProcess) gGaussianBlurVerticalPostProcess->Release();
